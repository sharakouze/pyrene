...
namespace Tmpi.Pyrene.Services.ServiceInterface
{
	...
	public partial class GenCompteurService : Service
	{
		...
		/// <returns></returns>
		public List<BasicEntity> Get(AutocompleteGenCompteur request)
		{
		...
		public GenCompteur Get(GetGenCompteur request)
		{
            ...
            {
                var undefFields = ModelDefinitionHelper.GetUndefinedFields<GenCompteur>(request.Fields);
                if (undefFields.Any())
                {
            ...
            var q = Db.From<GenCompteur>().Where(x => x.Id == request.Id).Select(request.Fields);
		...
		public GenCompteurValeur Get(GetGenCompteurValeur request)
		{
            if (!request.Fields.IsNullOrEmpty())
            {
                var undefFields = ModelDefinitionHelper.GetUndefinedFields<GenCompteurValeur>(request.Fields);
                if (undefFields.Any())
                {
                    string str = string.Join(", ", undefFields.Select(f => "'" + f + "'"));
                    throw new ArgumentException(
            ...
            var q = Db.From<GenCompteurValeur>().Where(x => x.Id == request.Id).Select(request.Fields);
		...
		public void Patch(PatchGenCompteur request)
		{
            ...
            var undefFields = ModelDefinitionHelper.GetUndefinedFields<GenCompteur>(patchDic.Keys);
            if (undefFields.Any())
            {
                string str = string.Join(", ", undefFields.Select(f => "'" + f + "'"));
                throw new ArgumentException(
			...
			var q = Db.From<GenCompteur>().Where(x => x.Id == request.Id).Update(patchDic.Keys);
		...
		public void Patch(PatchGenCompteurValeur request)
		{
            ...
            var undefFields = ModelDefinitionHelper.GetUndefinedFields<GenCompteurValeur>(patchDic.Keys);
            if (undefFields.Any())
            {
                string str = string.Join(", ", undefFields.Select(f => "'" + f + "'"));
                throw new ArgumentException(
			...
			var q = Db.From<GenCompteurValeur>().Where(x => x.Id == request.Id).Update(patchDic.Keys);
